	                    ...
	                    for (int xIter = minX; xIter < maxX; ++xIter) {
	                        ...
	                        for (int zIter = minZ; zIter < maxZ; ++zIter) {
	                            ...
	                            for (int yIter = maxY; yIter > minY; --yIter) {
	                                ...
	                                /*
	                                BlockState blockState1 = chunkPrimer.getBlockState(xIter, yIter, zIter);
	                                BlockState blockState2 = (BlockState)MoreObjects.firstNonNull(chunkPrimer.getBlockState(xIter, yIter + 1, zIter), BLK_AIR);
	
	
	                                if (this.isTopBlock(chunkPrimer, xIter, yIter, zIter, chunkX, chunkZ)) {
	                                    isTopBlock = true;
	                                }
	
	                                this.digBlock(chunkPrimer, xIter, yIter, zIter, chunkX, chunkZ, isTopBlock, blockState1, blockState2);
	                                */
	                                //.set(blockX, y, blockZ);
