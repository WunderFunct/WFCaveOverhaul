...
public class NURDynamicHolder {

    private List<NURDynamicLayer> riverLayers = new ArrayList<>();
    private boolean initialized = false;

    public static NURDynamicHolder INSTANCE = new NURDynamicHolder();
    ...
    public static void init(int global_min_y){
        ...
        if (global_min_y != -64) {
            ...
            for(int i = -56; i > global_min_y; i -= 16) {
                INSTANCE.addLayer(new NURDynamicLayer(Blocks.LAVA, -56, curOffset));
                curOffset += 3;
            }
    ...
    public void throwIfNotInitialized() {
        if (!this.initialized) {
            throw new NotImplementedException("Code error for Cave Overhaul. Attempted to run noise checks without NURDynamicHolder properly being initialized. Please inform W.F. of this error, preferably via commenting on the Curseforge mod page.");
        }
    }
    ...
    public boolean shouldSetToWater(int topY, int x, int y, int z) {
    ...
    public boolean shouldSetToLava(int topY, int x, int y, int z) {
        this.throwIfNotInitialized();
